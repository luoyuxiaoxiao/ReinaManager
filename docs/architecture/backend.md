...
少量启动设置使用 `tauri-plugin-store` 的 `settings.json`。封面、存档备份、数据库备份和安装中间文件存于文件系统。数据库备份目录在运行时解析失败时记录警告并回退默认目录，同时保留用户配置；存档备份根目录解析失败时直接报错，避免备份创建到无法由记录定位的目录。备份文件删除失败或文件不存在时保留数据库记录。
...
存档备份根目录变更必须调用 `change_savedata_backup_root`。该 command 与创建、恢复、删除和历史清理共用存档操作锁，按“复制到临时目录、逐文件校验、切换临时目录、更新配置、删除旧目录”的顺序执行；普通 `update_settings` 拒绝直接修改 `save_root_path`。迁移前会清理明确返回 `NotFound` 的单个失效备份记录；权限或网络错误不会触发清理。迁移失败时配置保持不变，旧目录不主动删除；配置已切换但旧目录清理失败时返回残留目录警告。旧根目录明确不存在但数据库仍有记录时，首次调用只返回确认要求；用户确认后才清理失效记录并切换配置。目录不可访问时不执行清理。新旧目录经词法规范化和符号链接解析后不得相同之外的父子重叠。
